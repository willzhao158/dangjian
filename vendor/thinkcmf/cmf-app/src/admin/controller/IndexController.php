...
class IndexController extends AdminBaseController
{
    ...
    cmf_clear_cache();
        ...
        $adminMenuModel = new \app\common\model\AdminMenuModel();
        $menus          = cache('admin_menus_' . cmf_get_current_admin_id(), '', null, 'admin_menus');

        if (empty($menus)) {
            $menus = $adminMenuModel->menuTree(cmf_get_current_admin_id()==1?133:0);
    ...
    public function massif(){
        $data = array(
            'park'  =>  DB::name('park')->field('id,parkname')->select(),
            'place' =>  DB::name('place')->field('id,parkid,plotname')->select(),
            'crop'  => DB::name('place_crop')->select()
        );
