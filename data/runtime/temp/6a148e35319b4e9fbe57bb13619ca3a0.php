<?php /*a:2:{s:85:"D:\xampp\htdocs\guocai.yzata.cn\public/themes/admin_simpleboot3/order\index\main.html";i:1579164374;s:80:"D:\xampp\htdocs\guocai.yzata.cn\public/themes/admin_simpleboot3/public\farm.html";i:1577019110;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
      ...
      <div class="layui-card">
        <div class="layui-card-body">
          <div class="demoTable">
            ...
            <button class="layui-btn " data-type="excelHref">导出订单</button>
            <?php endif; ?>
          </div>
        </div>
        <div class="layui-card-body">
...
.laytable-cell-1-0-14{overflow: hidden !important;}
.laytable-cell-1-0-24{overflow: hidden !important;} 
.laytable-cell-1-0-25{overflow: hidden !important;} 
</style>
...
<script type="text/html" id="barCol">
    ...
    <?php endif; ?>
    {{#  if(d.con_link != null){ }}
    <a class="layui-btn layui-btn-xs" lay-event="show_con">查看合同</a>
    {{#  } }} 
</script>
...
<script type="text/html" id="selpay_way" >    
  <select name='pay_way' lay-filter='pay_way' lay-event="pay_way" lay-search='' data-id="{{ d.id }}">
    <option value="" {{d.pay_way==''?'selected':''}}>请选择</option>
      ...
      <option value="富友" {{d.pay_way=='富友'?'selected':''}}>富友</option>
      <option value="续投" {{d.pay_way=='续投'?'selected':''}}>续投</option>
  </select>
</script>
...
layui.use(['laydate','table','layer','form'], function(){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'contra_num',  width:180,fixed: true, title: '合同编号111', <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ...
      ,{field:'annu_return', title: '年化收益',width:80, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ,{field:'sign_date', title: '签订日期',width:100,event: 'date', data_field: "dBeginDate"}
      ,{field:'inv_term', title: '投资期限',width:100,edit: 'text',toolbar: '#selinv_term'}
      ,{field:'deadline', title: '到期时间',width:100,event: 'deadline'}
      ,{field:'pay_time', title: '支付时间',width:100,event: 'pay_time'}
      ...
      ,{field:'id_num', title: '证件号码',width:200, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ,{field:'mobile', title: '手机号码',width:150, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ,{field:'inbank', title: '汇入银行',width:200, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ...
      ,{field:'pay_no', title: '参考号',width:120, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ,{field:'before_money', title: '本金提前转投金额',width:140, <?php if($user['level']!=='业务经理'): ?> edit: 'text'<?php endif; ?>}
      ...
      ,{field:'isdq', title: '是否到期',width:100}
  ...
  var $ = layui.$, active = {
    ...
    excelHref: function(){
      window.location.href = "<?php echo url('index/orderExcel'); ?>?company="+$('#company').val()+"&pro_name="+$('#pro_name').val()+"&contra_num="+$('#contra_num').val()+"&investor="+$('#investor').val()+"&adviser="+$('#adviser').val()+"&exam_status="+$('#exam_status').val()+"&sign_date="+$('#sign_date').val();
    },
    addPlace:function(){
